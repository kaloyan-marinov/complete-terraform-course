terraform {
  ...
  backend "s3" {
    ...
    key            = "07-managing-multiple-environments/option-2-file-structure/2-production/terraform.tfstate"
    region         = "us-east-1"
...
module "web_app" {
  ...
  # Input variables
  # bucket_name = "2-s3-bucket--07-managing-multiple-environments--option-2-file-structure-${local.environment_name}"
  bucket_name = "07-managing-multiple-environments--optn-2-file-struc-${local.environment_name}"
